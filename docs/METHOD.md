...
## Decisions locked (the bottom line, defend these)
...
Peng et al. 2019, arXiv:1910.00177) applied to a flow-matching policy — i.e. **ORW-CFM**
...
**Why it needs no action log-probabilities (the crux).** The flow-matching training loss is a
...
noise→action target velocity. It is an MSE in (state, noised-action, time) — **no normalizing
constant, no marginal likelihood of *a* anywhere in it.** Reweighting that loss by a
trajectory's return therefore needs only:

1. the ability to **sample** action chunks from the policy (forward ODE — that's just
   inference), and
...
**Contrast with PPO (the Q4 answer, sharpened).** PPO's objective is built on the probability
...
initialization and the entire ODE path that produced *a* — not cheaply or stably tractable.
*(Pedantic-but-honest caveat to volunteer: an exact likelihood does exist via the
...
so nobody forms PPO ratios that way in practice. This is exactly why the flow-RL literature
...
That is the whole point of Tier A: it sidesteps the intractable-likelihood problem rather than
...
## 2. The rollout → filter → fine-tune loop, concretely
...
2. **Filter / weight.** Keep the (obs, action) pairs from **successful** trajectories; discard
   failures. Assign each kept pair a weight `w` (see the collapse note below). → dataset `D_k`
...
3. **Fine-tune (GPU-bound, but short).** Continue-train `π_k` on `D_k` with the flow-matching
   ...
   inner steps (refinement, not a fresh 20k run). → `π_{k+1}`.
...
**The sparse-binary collapse — the honest core of this loop.** With reward `r ∈ {0,1}` and no
...
(offline/advantage-weighted) RL step — but with a pure 0/1 reward the *weighting* itself is
degenerate.
...
**Recommendation:** ship **pure success-filtering** as the honest sparse-reward baseline, and
...
The exact marginal `KL(π_θ ‖ π_SFT)` over action chunks needs both marginal likelihoods —
**intractable for the same reason PPO is** (§1). So the anchor must be a **tractable surrogate**
...
> fine-tune** — on tasks 0,9 it gave a clean FM-loss drop 1.3→0.09 (β=0) and the anchor
...
`π_SFT` because it's perpetually re-anchored to `π_SFT`'s data. Trivial to implement (two losses
...
same states) — weakest (sampling variance + extra `π_SFT` rollout cost). Mention, don't recommend.
...
|---|---|---|
| Rollouts per iteration, *N* | 50/task × 10 = **500** | At ~89% success that's ~445 successful trajectories → tens of thousands of (obs, chunk) pairs — plenty for filtered BC. Dominant sim-cost driver. |
...
to contain it: **batch-parallel envs** during collection (the 80 s/ep already assumes batch 4 —
...
**Why it's defensible (the key insight):** the RL headline is the **SFT→RL delta measured against
...
(the naive binomial is ≈±6, but episodes cluster within 10 tasks — within-task correlation widens
...
1. **Report per-task, not just aggregate — this is where the story lives.** The SFT per-task
   floor is **task 9 @ 60% (6/10)**, then **task 6 @ 70%**, **task 3 @ 80%**. Task 9 stayed at 6/10
   through SFT (it was the baseline's weak task too) — it is the natural RL target. If RL lifts
...
3. **Frame a flat result as a finding.** The headline is "SFT→RL delta **wherever it's
   ...
   OOD evidence"), and is consistent with the project's risk framing that an honest RL
...
| **Reward sparsity** | On **Goal**, the 89% policy yields ~445/500 successful rollouts — no starvation for filtered BC. **(Starvation IS real on `libero_10`** where success is low — see §8; that's why it's a probe, possibly demo-seeded.) |
| **Reward hacking** | The binary predicate is the **sim's ground-truth goal state** — far harder to hack than a learned reward. The *time-to-success* weight could be gamed (fast-but-reckless) → the anchor (§3) keeps it near demonstrated behavior. |
...
| **RL ≤ SFT (no gain)** | Not a project failure — it's the honest finding (§8, §10); the methodology + per-task/OOD analysis is the contribution (the project's risk framing). |
...
- **Volunteer the degenerate case:** under binary reward, "reward-weighted" BC **=** filtered BC
  (§2); the time-to-success weight is a mild, free signal that makes the weighting non-trivial —
  don't oversell it.
- **Volunteer the method's altitude (Q10):** this is **offline/advantage-weighted "RL-lite,"** not
  on-policy policy gradient — legitimate RL (signal = environment reward on the policy's own
...
4. `results/rl_libero_goal.json` (same schema as `sft_libero_goal.json`) + a writeup section whose
   honest framing — including a flat/negative result if that's what happens — matches
   `FAQ.md`.
