...
- Verify keys/shapes match the policy (two cameras → `observation.images.image`
  / `image2`, `observation.state` 8-dim) — expected to match (Phase 2 confirmed
  env-side), but the training data path is new.
...
- Measure: VRAM headroom at batch 64 (drop to 32 if needed), steps/sec, and
  **compute-unit burn rate** (read the Colab meter before/after — this calibrates
...
- Optional (cheap insight, +2.5 h): eval `lerobot/smolvla_base` zero-shot for
  the full zero-shot → SFT delta. Nice for the writeup.
...
- **Dataset delivery**: 34GB repo, filtered to ~"goal" episodes. Colab runtime
  disk holds it, but it re-downloads each session (~15–30 min) — acceptable;
  do NOT put the HF cache on Drive (many-small-files IO is terrible).
  Step 1 determines whether `--dataset.episodes` filtering downloads only the
  needed shards or the whole set.
- **Flag drift**: `--dataset.episodes` syntax (draccus list — brackets, per the
  Phase-2 lesson) and `--resume` semantics need the step-0 reconcile.
- **Batch 64 VRAM on A100 40GB**: expected to fit (~450M params + frozen
  vision encoder), but unverified → smoke run.
